...
export const actions: Actions = {
  default: async ({ request, cookies, getClientAddress }) => {
    ...
    if (action === 'login_password') {
      return handleLoginPassword(data, cookies, getClientAddress);
    }

    // ── Request membership (no account yet) ──
    if (action === 'request_membership') {
      return handleMembershipRequest(data);
    }

    // No action specified — return error
    return fail(400, { error: 'Ingen åtgärd angiven.' });
  },
};

// ── Helper: Password-based login ────────────────────────────────────────
async function handleLoginPassword(
  ...
  const inviteToken = (data.get('invite_token') as string)?.trim() || undefined;
  ...
  // Redirect: invite context → register or confirm, otherwise vineyard
  if (inviteToken) {
    throw redirect(303, `/register?invite=${encodeURIComponent(inviteToken)}`);
  }
  throw redirect(303, '/vineyard');
}

// ── Helper: Membership request ──────────────────────────────────────────
async function handleMembershipRequest(data: FormData) {
  const email = (data.get('email') as string)?.trim().toLowerCase();
