...
**Predecessor:** `architecture.md` (SvelteKit-era, superseded). This spec replaces all prior documentation for the project.
...
Svenskt Vin is a **standalone single-binary web application** for viticulture community management. It is an **artifact** of the Cinerarium platform, not a platform component. It has zero dependency on Cinerarium's TAP-Core, MCP Gateway, or agent infrastructure.
...
<head>
    ...
    <!-- HTMX -->
    <script src="https://unpkg.com/htmx.org@2.0.4" integrity="sha384-HGfztofotfMzorYmKGrM1lGbI0i6YcfnBDWTRfz94B0nTDnO2UcBZcDR40XyXGNq" 
...
- **Password hashing:** bcrypt, minimum 12 rounds. Never store plaintext.
- **password_hash never exposed:** Never returned in API responses, session payloads, or log output.

### 11.2 Session Security
- **Session cookies:** `httpOnly`, `secure`, `SameSite=Lax`. Session ID is cryptographically random.
- **Session expiry:** 30 days, renewed on activity.
- **Standalone DB:** Svenskt Vin uses its own PostgreSQL instance. No connection to Cinerarium's TAP-Core.
- **No MCP tools:** Go handlers use pgx directly. The MCP `db_query`/`db_exec` tools are for Cinerarium agents only, not for the web app.

### 11.3 Rate Limiting
- Login and password reset endpoints rate-limited per IP (token bucket, ~5 attempts per 15 min).
- In-memory rate limiter; Redis needed for multi-replica deployments.

### 11.4 Enumeration Prevention
- "Forgot password" returns same message for existing/non-existing email.
...
## 14. Data Migration (Legacy Users)
